...
def write_json(all_results, outputpath, form=None, for_eval=False, opt=None):
    '''
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            if opt.format == 'vidor':
                ...
                tid = int(human['idx'])
                if tid not in pose_results:
                    pose_results[tid] = {}
                frame_idx = int(im_name.split('.')[0].split('_')[1]) - 1
                pose_results[tid][frame_idx] = human['keypoints'][:, :2].tolist()
            else:
                keypoints = []
                result = {}
                if for_eval:
    ...
    if form == 'cmu': # the form of CMU-Pose
        ...
        # print([(data, type(pose_results[data])) for data in pose_results.keys()])
        with open(os.path.join(outputpath), 'w') as json_file:
            json_file.write(json.dumps(pose_results))
    else:
        with open(os.path.join(outputpath,'alphapose-results.json'), 'w') as json_file:
